...
public abstract class Coordinates : KmlObjectBase
{
	...
	public Coordinates(IEnumerable<KmlPosition> coordinatesArray)
	{
        coordinates = string.Join(" ", coordinatesArray.Select(c =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(c.altitude) && !"0".Equals(c.altitude, StringComparison.Ordinal))
            {
                list.Add(c.altitude);
            }
	...
	public KmlPosition[] ToArray()
	{
		...
		return Array.ConvertAll(coordinates.Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries), coord =>
		{
			var array = coord.Split(new string[1] { "," }, StringSplitOptions.RemoveEmptyEntries);
...
public class MultiGeometry : Coordinates
{
	...
	public Coordinates[] Items { get; set; }
}
