...
internal static class Win32API
{
    ...
    public class SafeFileMappingHandle : SafeKernelObjectHandle
    {
        ...
        public SafeFileMappingHandle(nint open_handle, bool owns_handle) : base(open_handle, owns_handle)
        {
        }
    ...
    public class SafeKernelObjectHandle : SafeHandleZeroOrMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeKernelObjectHandle(nint open_handle, bool owns_handle) : base(owns_handle)
        {
        ...
        : SafeHandleZeroOrMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeLocalAllocBuffer(nint open_handle, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
            Initialize((ulong)LocalSize(this));
        }
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeLocalAllocBuffer(nint open_handle, ulong numBytes, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
            Initialize(numBytes);
        }
    ...
    public class SafeServiceHandle : SafeHandleZeroOrMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeServiceHandle(nint open_handle, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
        }
    ...
    [DllImport("advapi32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern bool StartService(SafeServiceHandle hService, int dwNumServiceArgs, nint lpServiceArgVectors);
    ...
    public class SafeFindVolumeHandle : SafeHandleMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeFindVolumeHandle(nint open_handle, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
        }
    ...
    public class SafeFindVolumeMountPointHandle : SafeHandleMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeFindVolumeMountPointHandle(nint open_handle, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
        }
    ...
    public class SafeFindHandle : SafeHandleMinusOneIsInvalid
    {
        ...
        /// instance is released.</param>
        [SecurityCritical]
        public SafeFindHandle(nint open_handle, bool owns_handle) : base(owns_handle)
        {
            SetHandle(open_handle);
        }
    ...
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, nint lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true, EntryPoint = "DeviceIoControl")]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, in ushort lpInBuffer, uint nInBufferSize, nint lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true, EntryPoint = "DeviceIoControl")]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, [MarshalAs(UnmanagedType.I1)] in bool lpInBuffer, uint nInBufferSize, nint lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true, EntryPoint = "DeviceIoControl")]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, out long lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, out NativeConstants.PARTITION_INFORMATION lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, out NativeConstants.PARTITION_INFORMATION_EX lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, out NativeConstants.DISK_GEOMETRY lpOutBuffer, int nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, int nInBufferSize, out NativeConstants.SCSI_ADDRESS lpOutBuffer, int nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, out NativeConstants.STORAGE_DEVICE_NUMBER lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern bool CreateDirectory([MarshalAs(UnmanagedType.LPTStr)][In] string lpszPathName, nint lpSecurityAttributes);
    ...
    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool SetFilePointerEx(SafeFileHandle hFile, long distance_to_move, nint ptr_new_file_pointer, uint move_method);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern SafeFileHandle CreateFile([In][MarshalAs(UnmanagedType.LPTStr)] string lpFileName, uint dwDesiredAccess, uint dwShareMode, nint lpSecurityAttributes, uint dwCreationDisposition, int dwFlagsAndAttributes, nint hTemplateFile);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool GetFileSizeEx(SafeFileHandle hFile, out long liFileSize);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, uint nInBufferSize, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 6)] byte[] lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, [MarshalAs(UnmanagedType.LPArray)][In()] byte[] lpInBuffer, uint nInBufferSize, nint lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, [MarshalAs(UnmanagedType.LPArray)][In()] byte[] lpInBuffer, uint nInBufferSize, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 6)] byte[] lpOutBuffer, uint nOutBufferSize, out uint lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true)]
    internal static extern bool DeviceIoControl(SafeFileHandle hDevice, uint dwIoControlCode, nint lpInBuffer, int nInBufferSize, out NativeConstants.DiskExtents lpOutBuffer, int nOutBufferSize, out int lpBytesReturned, nint lpOverlapped);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern nint GetModuleHandle([In][MarshalAs(UnmanagedType.LPTStr)] string lpModuleName);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern int GetModuleFileName(nint hModule, StringBuilder lpFilename, int nSize);
    ...
    public static extern nint GetProcAddress(nint hModule, [In][MarshalAs(UnmanagedType.LPStr)] string lpEntryName);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Ansi)]
    public static extern nint GetProcAddress(nint hModule, nint ordinal);
    ...
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern nint LocalSize([In] SafeHandle hMem);
    ...
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern nint LoadLibrary([MarshalAs(UnmanagedType.LPTStr)][In] string lpFileName);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern nint LoadLibraryEx([MarshalAs(UnmanagedType.LPTStr)][In] string lpFileName, nint hFile, uint dwFlags);
    ...
    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern NativeConstants.Win32FileType GetFileType(nint handle);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern NativeConstants.Win32FileType GetFileType(SafeFileHandle handle);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern nint GetStdHandle(NativeConstants.StdHandle nStdHandle);

    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool GetConsoleScreenBufferInfo(nint hConsoleOutput, out NativeConstants.CONSOLE_SCREEN_BUFFER_INFO lpConsoleScreenBufferInfo);
    ...
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern SafeFileMappingHandle CreateFileMapping(nint p1, in NativeConstants.SECURITY_ATTRIBUTES p2, NativeConstants.PageProtection p3, uint p4, uint p5, [MarshalAs(UnmanagedType.LPTStr)][In] string p6);
    ...
    // [DllImport Lib "kernel32" ("kernel32.dll", CallingConvention = CallingConvention.Winapi, SetLastError = true)]
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern nint MapViewOfFile(SafeFileMappingHandle p1, NativeConstants.MemoryMappedFileRights p2, uint p3, uint p4, UIntPtr p5);
    ...
    public static extern int GetMappedFileName(nint p1, nint p2, [MarshalAs(UnmanagedType.LPTStr)] string p3, int p4);
    [DllImport("psapi", EntryPoint = "GetMappedFileNameW", CharSet = CharSet.Unicode)]
    public static extern int GetMappedFileName(nint p1, nint p2, [MarshalAs(UnmanagedType.LPArray)] char[] p3, int p4);
    [DllImport("psapi", EntryPoint = "GetMappedFileNameW", CharSet = CharSet.Unicode)]
    public static extern int GetMappedFileName(nint p1, nint p2, [MarshalAs(UnmanagedType.LPWStr)] StringBuilder p3, int p4);
    ...
    // [DllImport Lib "kernel32" ("kernel32.dll", CallingConvention = CallingConvention.Winapi, SetLastError = true)]
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern UIntPtr VirtualQuery(nint p1, out NativeConstants.MEMORY_BASIC_INFORMATION p2, UIntPtr p3);

    // [DllImport Lib "kernel32" ("kernel32.dll", CallingConvention = CallingConvention.Winapi, SetLastError = true)]
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern bool FlushViewOfFile(nint p1, uint p2);

    // [DllImport Lib "kernel32" ("kernel32.dll", CallingConvention = CallingConvention.Winapi, SetLastError = true)]
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern bool UnmapViewOfFile(nint p1);
    ...
    public static extern bool RtlDosPathNameToNtPathName_U([MarshalAs(UnmanagedType.LPTStr)][In] string DosName, out NativeConstants.UNICODE_STRING NtName, nint DosFilePath, nint NtFilePath);

    [DllImport("ntdll.dll")]
    public static extern void RtlFreeUnicodeString(in NativeConstants.UNICODE_STRING UnicodeString);

    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    public static extern int SearchPath([MarshalAs(UnmanagedType.LPTStr)][In] string lpPath, [MarshalAs(UnmanagedType.LPTStr)][In] string lpFileName, [MarshalAs(UnmanagedType.LPTStr)][In] string lpExtension, int nBufferLength, [MarshalAs(UnmanagedType.LPTStr), Out] StringBuilder lpBuffer, out nint lpFilePart);
    ...
    [DllImport("advapi32.dll", SetLastError = true, EntryPoint = "SystemFunction036", CharSet = CharSet.Auto)]
    public static extern byte RtlGenRandom(nint buffer, int length);
    ...
    [DllImport("user32.dll", SetLastError = true, EntryPoint = "SystemParametersInfoA", CharSet = CharSet.Ansi)]
    public static extern int SetSystemParametersDWORD(uint uAction, uint uParam, nint lpvParam, uint fuWinIni);

    [DllImport("oleaut32.dll", SetLastError = true)]
    public static extern int SysStringByteLen(nint strptr);

    [DllImport("oleaut32.dll", SetLastError = true)]
    public static extern nint SysAllocStringByteLen([In][MarshalAs(UnmanagedType.LPArray)] byte[] psz, int len);
